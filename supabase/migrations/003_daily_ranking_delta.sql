create or replace function public.ranking_daily_delta(p_day date default ((now() at time zone 'Europe/Madrid')::date - 1))
returns table (
  user_id uuid,
  display_name text,
...
as $$
  with day_points as (
    ...
    where
      m.status = 'FINAL'
      and (m.kickoff_at at time zone 'Europe/Madrid')::date = p_day
    group by pr.user_id
  ),
  scores as (
    select
